	...
	const FString& OldTag, const FString& NewTag, bool bRenameChildren)
{
	...
	// UE 5.6+: RenameTagInINI only takes (TagToRename, TagToRenameTo); third arg removed from API.
	if (bRenameChildren)
	{
		UE_LOG(LogTemp, Verbose, TEXT("RenameGameplayTag: bRenameChildren unsupported by GameplayTagsEditor API on this engine; performing single-tag rename."));
	}
