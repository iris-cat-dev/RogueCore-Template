	...
	const FString& FilePath)
{
	...
	if (!SkeletalMeshPath.Contains(TEXT("SK_BACKTRACKER")))
	{
		BridgeLevelImpl::AttachPreviewAssets(SkelComp, Anim->GetSkeleton(), PreviewScene);
	}
	if (SkeletalMeshPath.Contains(TEXT("SK_BACKTRACKER")))
	{
		const int32 HiddenMaterialId = FilePath.Contains(TEXT("hide_slot0")) ? 0 : (FilePath.Contains(TEXT("hide_slot1")) ? 1 : INDEX_NONE);
		if (HiddenMaterialId != INDEX_NONE)
		{
			for (int32 SectionIndex = 0; SectionIndex < 32; ++SectionIndex)
			{
				SkelComp->ShowMaterialSection(HiddenMaterialId, SectionIndex, false, 0);
			}
		}
	}
